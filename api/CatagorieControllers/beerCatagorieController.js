...
const getMany = (req, res) => {
  connection.query(`select * from beer_catagories`, (err, result) => {
    if (err){
      res.status(400).json('Error returning all beer catagories')
    }
    else{
...
const createOne = (req, res) => {
  ...
  connection.query('insert into beer_catagories set ?', catagorie, (err, result) => {
    if (err){
      ...
      console.log(result)
      res.status(200).json('Beer catagorie added')
    }
  })
...
if(!catagorie){
  ...
  connection.query(query,[catagorie, id], (err, result) => {
    if(err){
      res.status(400).json('Error updating beer catagorie')
    }
    else {
      res.status(200).json('Beer catagorie has been updated')
    }
  })
  ...
  if(!isNaN(del)){
    ...
    (err, result) => {
      ...
      if(err){
        ...
        res.status(200).json('Beer catagorie deleted')
      }
