...
const getOne = (req, res) => {
  ...
  connection.query(
    ...
    where wine_region_id = ${id}`, (err, result) => {
      if (err) throw err
...
if(!catagorie){
  ...
  const query = 'UPDATE wine_region SET colour WHERE wine_region_id = ?'
...
const deleteOne = (req, res) => {
  ...
  if(!isNaN(del)){
    connection.query(`delete from wine_region where wine_region_id = ${del}`, 
    (err, result) => {
