...
const getOne = (req, res) => {
  ...
  connection.query(
    ...
    where spirit_catagorie_id = ${id}`, (err, result) => {
      if (err) throw err
...
if(!spirit_type){
  ...
  const query = 'UPDATE spirit_catagories SET spirit_type WHERE spirit_catagorie_id = ?'
...
const deleteOne = (req, res) => {
  ...
  if(!isNaN(del)){
    connection.query(`delete from spirit_catagories where spirit_catagorie_id = ${del}`, 
    (err, result) => {
